...
describe("Health check", () => {
  it("should return ok when server alive", async () => {
    const res = await app.handle(new Request("http://localhost/api/health/check-server"));

    expect(res.status).toBe(200);
  });

  it("should return ok when database server alive", async () => {
    const res = await app.handle(new Request("http://localhost/api/health/check-database"));
